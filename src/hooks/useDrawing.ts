...
import { ShapeStyle } from "../data";
import { getSelectAreaSize } from "../helpers/getSelectAreaSize";
import { getRelativePointerPosition } from "../helpers/getRelativePointerPosition";
import { figureSize } from "../helpers/figureSize";
...
export const useDrawing = ({
  ...
  const onMouseMove = (e: KonvaEventObject<MouseEvent>) => {
    ...
    const { height, width, x, y } = getSelectAreaSize(pos, {
      x: selectedArea.startX,
      y: selectedArea.startY,
    });

    const rectSelection = figureSize.getRectSize({ height, width }, { x, y });
    ...
    if (tool === Tool.PENCIL && shape.type === ShapeType.PENCIL) {
    ...
    if (tool === Tool.CIRCLE) {
      const newSize = figureSize.getCircleSize({ height, width }, { x, y });
